...
from src.utils import search_to_df, get_graphistry_from_search, \
    get_graphistry_from_milieu_search, setup_logger
...
def display_graph(g):
    ...
    st.markdown(f'<iframe width=1000 height=800 src="{url}"></iframe>', unsafe_allow_html=True)
...
def milieu_search(search_term, both=False):
    g = get_graphistry_from_milieu_search(search_term, src, dst, node_col, edf, ndf, both=both)
    return g

@st.cache
def text_search(search_term):
    res = pd.concat([search_to_df(search_term, 'Summary', ndf), search_to_df(search_term, 'Blurb', ndf)], axis=0)
